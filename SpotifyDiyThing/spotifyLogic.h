...
void spotifySetup(SpotifyDisplay *theDisplay, const char *clientId, const char *clientSecret)
{
  ...
  // SpotifyArduinoCert.h pins i.scdn.co (album art) to the same DigiCert Global
  // Root G2 cert as the API hosts, but i.scdn.co's chain (DigiCert Global G3 TLS
  // ECC SHA384 2020 CA1, confirmed live via openssl) verifies only against
  // DigiCert Global Root G3 - a different, unrelated root. Without this override
  // every album art download fails X.509 verification (imageStatus: -2) on every
  // board. Overridden here, once, since all display files share this global.
  spotify_image_server_cert = "-----BEGIN CERTIFICATE-----\n"
